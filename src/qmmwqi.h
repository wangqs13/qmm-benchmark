...
public:
    ...
    friend std::pair<bool, Configuration> check(const QuantumMealyMachineWithQuantumInput &m1, const matrix &rho1, const QuantumMealyMachineWithQuantumInput &m2, const matrix &rho2, const std::vector<matrix> &basis, AlmostEqual<V> equal)
    {
        ...
        while (!Q.empty())
        {
            ...
            //matrix rho = config.rho1-config.rho2;
            if (ortho.is_independent(rho))
            {
                //std::cout << config << " " << rho << std::endl;
    ...
    friend std::pair<bool, Configuration> check_slow(const QuantumMealyMachineWithQuantumInput &m1, const matrix &rho1, const QuantumMealyMachineWithQuantumInput &m2, const matrix &rho2, const std::vector<matrix> &basis, AlmostEqual<V> equal)
    {
        assert(m1.dim_in == m2.dim_in && m1.gamma == m2.gamma);
        int dim_in = m1.dim_in;
        int gamma = m1.gamma;
        ...
        while (!Q.empty())
        {
            ...
            Q.pop();
            matrix rho = direct_sum(config.rho1, -config.rho2);
            if (ortho.is_independent_slow(rho))
            {
